...
func TestPatternMatch(t *testing.T) {
	...
	if !PattenMatch(p5, s5) {
		t.Error("PattenMatch(\"h[ae]llo\", \"hello\") should return true")
	}
	if !PattenMatch(p5, s5_1) {
		t.Error("PattenMatch(\"h[ae]llo\", \"hallo\") should return true")
	}
	...
	if !PattenMatch(p7, s7) {
		t.Error("PattenMatch(\"h[a-c]llo\", \"hallo\") should return true")
	}
	if !PattenMatch(p7, s7_1) {
		t.Error("PattenMatch(\"h[a-c]llo\", \"hbllo\") should return true")
	}
